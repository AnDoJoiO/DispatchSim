    ...
    <div class="db-modal-body flex flex-1 overflow-hidden" style="background:var(--bg)">
      ...
      >
        <div
          ...
          style="background:var(--surface-raised);border-bottom:1px solid var(--border)"
        >
        ...
        >
          <!-- Loading -->
          <div v-if="loading" class="m-auto text-center" style="color:var(--text-muted)">
          ...
          <!-- Empty -->
          <p v-else-if="!data?.transcript?.length" class="text-xs m-auto" style="color:var(--text-muted)">
            {{ tr('dm.no_messages') }}
          </p>
          <!-- Messages -->
          <template v-else>
      ...
      >
        <div class="p-4" style="border-bottom:1px solid var(--border)">
          <h3 class="text-xs font-bold uppercase tracking-widest mb-3" style="color:var(--text-muted)">{{ tr('dm.metrics') }}</h3>
          <div class="flex flex-col gap-2">

            <div class="metric-card flex justify-between items-center p-3 rounded-xl" style="background:var(--surface-raised);border:1px solid var(--border)">
              <div>
                <p class="text-xs font-semibold" style="color:var(--text-secondary)">{{ tr('dm.duration') }}</p>
                <p class="text-xs mt-0.5" style="color:var(--text-muted)">{{ tr('dm.duration_sub') }}</p>
              </div>
            ...
            <div class="metric-card flex justify-between items-center p-3 rounded-xl" style="background:var(--surface-raised);border:1px solid var(--border)">
              <div>
                <p class="text-xs font-semibold" style="color:var(--text-secondary)">{{ tr('dm.response') }}</p>
                <p class="text-xs mt-0.5" style="color:var(--text-muted)">{{ tr('dm.response_sub') }}</p>
              </div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <div class="metric-card p-3 rounded-xl text-center" style="background:var(--surface-raised);border:1px solid var(--border)">
                <p class="text-xs mb-1" style="color:var(--text-muted)">{{ tr('dm.msg_count') }}</p>
                <p class="font-mono text-xl font-bold" style="color:var(--text)">{{ data?.message_count ?? '—' }}</p>
              </div>
              <div class="metric-card p-3 rounded-xl text-center" style="background:var(--surface-raised);border:1px solid var(--border)">
                <p class="text-xs mb-1" style="color:var(--text-muted)">{{ tr('dm.priority') }}</p>
                <p
            ...
            <div class="metric-card p-3 rounded-xl" style="background:var(--surface-raised);border:1px solid var(--border)">
              <p class="text-xs" style="color:var(--text-muted)">{{ tr('dm.call_start') }}</p>
        ...
        <!-- Fitxa data -->
        <div class="p-4">
          <h3 class="text-xs font-bold uppercase tracking-widest mb-3" style="color:var(--text-muted)">{{ tr('dm.data_sheet') }}</h3>
          <div class="flex flex-col rounded-xl overflow-hidden" style="border:1px solid var(--border)">
            ...
            <template v-else>
              <div class="fitxa-row-odd flex justify-between items-start px-4 py-3"
                   style="background:var(--surface-raised);border-bottom:1px solid var(--border2)">
                <span class="text-xs" style="color:var(--text-muted)">{{ tr('dm.address') }}</span>
              ...
              <div class="fitxa-row-even flex justify-between items-center px-4 py-3"
                ...
                <span class="text-xs" style="color:var(--text-muted)">{{ tr('dm.phone') }}</span>
              ...
              <div class="fitxa-row-odd flex justify-between items-center px-4 py-3"
                   style="background:var(--surface-raised);border-bottom:1px solid var(--border2)">
                <span class="text-xs" style="color:var(--text-muted)">{{ tr('dm.injured') }}</span>
                <span
                  class="text-base font-bold font-mono"
                  :style="data.intervention.num_injured > 0 ? 'color:#ef4444' : 'color:var(--text-muted)'"
              ...
              <div
                ...
                style="background:var(--surface);border-bottom:1px solid var(--border2)"
              >
                <p class="text-xs mb-2" style="color:var(--text-muted)">{{ tr('dm.risks') }}</p>
              ...
              <div
                ...
                style="background:var(--surface-raised);border-bottom:1px solid var(--border2)"
              >
                <p class="text-xs mb-1.5" style="color:var(--text-muted)">{{ tr('dm.notes') }}</p>
                <p class="text-xs leading-relaxed" style="color:var(--text-secondary)">{{ data.intervention.operator_notes }}</p>
              </div>
              <div class="fitxa-db-foot px-4 py-2 text-right" style="background:var(--surface)">
                <p class="text-xs" style="color:var(--text-muted)">{{ tr('dm.saved') }} {{ fmtDate(data.intervention.saved_at) }}</p>
              </div>
            </template>
          </div>
        </div>
